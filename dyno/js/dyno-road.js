...
(function(){
  ...
  async function isConnected(){
    // Prioritas: DYNO_getConn_DUAL() -> lebih ringan
    try{
      if (typeof window.DYNO_getConn_DUAL === "function"){
        const c = await window.DYNO_getConn_DUAL();
        return !!(c && (c.connected || c.online));
      }
    }catch(e){}

    // Fallback: coba snapshot (kalau ada snapshot berarti modul reachable)
    try{
  ...
  async function requireConnected(actionLabel){
    ...
    if (!ok){
      ...
      setStatus(String(actionLabel || "AKSI") + ": MODUL TIDAK TERHUBUNG");
  ...
  window.DYNO_arm = async function(){
    if (!(await requireConnected("ARM"))) return;   // ✅ wajib terhubung
  ...
  window.DYNO_read = async function(){
    if (!(await requireConnected("READ"))) return;  // ✅ wajib terhubung
  ...
  window.DYNO_run = async function(){
    if (!(await requireConnected("RUN"))) return;   // ✅ wajib terhubung
  ...
  window.DYNO_stop = async function(){
    if (!(await requireConnected("RESET"))) return; // ✅ wajib terhubung
  ...
  window.DYNO_saveCSV = async function(){
    if (!(await requireConnected("SAVE CSV"))) return; // ✅ wajib terhubung
  ...
  function debouncePushConfigToESP(){
    ...
    DYNO.cfgPushTimer = setTimeout(async () => {
      ...
      // ✅ kalau mau config juga wajib terhubung, aktifkan 2 baris ini:
      // if (!(await requireConnected("CONFIG"))) return;
  ...
  function createLogDomAtTop(log){
    ...
    btn.type = "button";
    btn.onclick = async () => {
      if (!(await requireConnected("SAVE LOG"))) return; // ✅ wajib terhubung
      saveOneLogCSV(log);
    };
